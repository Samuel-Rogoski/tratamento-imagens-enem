...
def encontrar_borda_inferior(imagem, cor_alvo=(64, 193, 243), tolerancia=15):
    ...
    # Percorre a imagem de baixo para cima (garante espaço para os 12 pixels do padrão)
    for y in range(altura - 1, 12, -1):
        ...
        # Se passou por todas as validações, a borda completa foi achada
        if borda_encontrada:
            # O topo da borda está em y-11. O pixel logo acima dela é y-12
            posicao_corte = y - 12
            print(f"Borda encontrada! Cortando na posição y={posicao_corte}")
        ...
        try:
            ...
            with Image.open(caminho_origem) as imagem:
                ...
                if posicao_corte is not None and posicao_corte > 0:
                    # Se encontrou a borda, recorta a imagem da linha 0 até a linha do corte
